...
public class EmployeeController {
    ...
    //@Controller:  <- MPA(Server Side Rendering)
    //      @GetMapping, @PostMapping 요청만 받는다. 백엔드 로직 처리, 뷰페이지를 리턴
    //@RestController: SPA(Client Side Rendering)
    //   JSON으로 값을 클라이언트에 던져줌. Client에서 JSON으로 값을 받아서 DB에 저장함
    //      @GetMapping(DB에 Select),
    //      @PostMapping(DB에 Insert),
    //      @PutMapping(DB에 Update),
    //      @DeleteMapping(DB에 Delete)
